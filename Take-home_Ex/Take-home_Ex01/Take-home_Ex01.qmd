...
There are 4 different automatic bandwidth selection method that we can choose: *bw.diggle, bw.CvL, bw.scott* and *bw.ppl*. For this exercise, we are using *bw.diggle*, or the radius selection method. (should use bw.ppl as it tends to produce the more appropriate values when the pattern consists predominantly of tight clusters.) We can retrieve the bandwidth used to compute the KDE layer as well.

```{r}
bw <- bw.diggle(originSG)
bw
```

We can plot the KDE in order to see what it looks like on the map. As a reminder, the current data frame contains 280 000 items, so the numbers may be fairly large.
...
plot(density(originSG_km, 
     ...
     main="Disc")
```
Since there is not much of a difference between the different kernel methods, we will simply use the default Gaussian method.

# Fixed and Adaptive KDE bandwidths


# Other attempts at bandwidth
```{r}
#| eval: false
