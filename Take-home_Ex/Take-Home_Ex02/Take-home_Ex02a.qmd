...
Before a relational join can be conducted, I will first need to rename the relevant/important attribute rows. The following rows will be renamed accordingly into Simplified Chinese using the *colnames()* function:

| Original | Renamed                 |
|----------|-------------------------|
| TOWNNAME | 居住鄉鎮                |
| VILLNAME | 居住村里                |
| geometry | geometry (must be kept) |
...
```{r}
tainan_cases <- left_join(cases,tainan)
```
